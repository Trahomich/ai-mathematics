...
## 1. Что значит «модель ошибается»

Прежде чем спускаться по градиенту, нужно понять **что именно мы минимизируем**.

Любое обучение нейросети = минимизация функции потерь (loss function). Она измеряет, насколько предсказание $\hat{y}$ далеко от правильного ответа $y$.

---

## 2. MSE: квадратичная цена ошибки

**Mean Squared Error** — стандартная функция потерь для регрессии:
...
- Квадрат сильнее штрафует большие ошибки: ошибка 10 даёт штраф 100, а не 10
- Квадрат гладкий — производная существует везде (модуль имеет излом в нуле)
- Аналитическое решение: $w^* = (X^TX)^{-1}X^Ty$

### Пример

Предсказываем цену квартиры. Модель: $\hat{y} = 250 \cdot x_1 + 50 \cdot x_2 + 30$

| Реальная цена | Предсказание | Ошибка | Квадрат |
|---|---|---|---|
| 500 | 480 | 20 | 400 |
| 300 | 350 | -50 | 2500 |
| 700 | 690 | 10 | 100 |

$MSE = (400 + 2500 + 100) / 3 = 1000$

---

## 3. Cross-entropy: логарифмический штраф

**Кросс-энтропия** — функция потерь для классификации:
...
Если правильный класс = 1, а модель предсказала $\hat{y} = 0.99$:
- loss = $-\log(0.99) = 0.01$ — почти ноль, модель уверена правильно

Если модель предсказала $\hat{y} = 0.01$:
- loss = $-\log(0.01) = 4.6$ — большой штраф, модель ошиблась уверенно

**Ключевое свойство:** логарифм экспоненциально штрафует уверенные ошибки.

### Связь с softmax

Для многоклассовой классификации softmax + cross-entropy:

$$
\hat{y}_k = \frac{e^{z_k}}{\sum_{j=1}^{C} e^{z_j}}, \quad \mathcal{L} = -\log(\hat{y}_{\text{correct}})
$$

Градиент по логитам: $\frac{\partial \mathcal{L}}{\partial z_k} = \hat{y}_k - y_k$ — элегантно и просто.
...
## 5. Градиентный спуск: базовый алгоритм
...
- $\eta = 0.1$: $w_0=3 \to 2.4 \to 1.92 \to 1.54 \to ... \to 0$ — медленно но верно
- $\eta = 1.0$: $w_0=3 \to -3 \to 3 \to -3 \to ...$ — осцилляция
- $\eta = 2.0$: $w_0=3 \to -9 \to 27 \to ...$ — расходимость

### 2D-ландшафт потерь: живая визуализация

Рассмотрим функцию потерь от двух весов: $\mathcal{L}(w_1, w_2) = w_1^2 + 3w_2^2$
...
Контуры (линии равного loss):

- Внешний контур: $\mathcal{L} = 9$ (эллипс вытянут по $w_1$ — овраг вдоль $w_1$)
- Средний контур: $\mathcal{L} = 3$
- Внутренний контур: $\mathcal{L} = 1$

**Траектория градиентного спуска** ($\eta = 0.2$, старт $w_0 = (2.5, 2.0)$):

```
Шаг 0: (2.50, 2.00)  loss = 18.25
Шаг 1: (1.50, -0.40)  loss = 2.49    ← перепрыгнули через минимум по w₂!
Шаг 2: (0.90,  0.56)  loss = 1.72    ← зигзаг
Шаг 3: (0.54, -0.11)  loss = 0.33
Шаг 4: (0.32,  0.15)  loss = 0.13
...
Шаг 12: (0.00,  0.00)  loss ≈ 0
```

**Вывод:** из-за разного масштаба ($1$ vs $3$) по осям спуск зигзагает. Это мотивация для:
- Нормализации данных (лекция 3)
- Momentum (секция 8) — гасит зигзаг
- Adam (секция 9) — адаптирует learning rate по каждой оси
