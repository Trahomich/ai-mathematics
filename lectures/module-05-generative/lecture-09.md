...
**Определение.** Пара стратегий $(G^*, D^*)$ — равновесие Нэша, если ни один игрок не может улучшить свой результат, изменив стратегию в одиночку.

Для GAN равновесие достигается когда:

$$
p_g = p_{data} \quad \text{и} \quad D^*(x) = \frac{1}{2} \;\forall x
$$
...
|---|---|
| $G$ пытается улучшиться при $D = D^*$ | $p_g$ уже совпадает с $p_{data}$ — некуда улучшаться |
| $D$ пытается улучшиться при $G = G^*$ | $p_g = p_{data}$ — максимум что может $D$ это угадывать с вероятностью 0.5 |

**На практике** равновесие Нэша почти никогда не достигается. GAN обучаются вблизи равновесия — дискриминатор колеблется вокруг 0.5, а генератор продолжает дрейфовать.

Мониторинг: если $D$ стабильно выдает $\neq 0.5$ — обучение развалилось:
- $D \approx 1.0$ → генератор порождает мусор
- $D \approx 0.0$ → дискриминатор побеждён (mode collapse возможен)
- $D \approx 0.5$ → область равновесия, обучение идёт нормально
